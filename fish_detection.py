...
class EnhancedFisheriesSystem:
    def __init__(self):
        ...
        self.weather_data = self.get_coastal_weather(*self.current_location)  # Fixed argument passing
        self.fishing_status = self.check_fishing_ban()
        self.ocean_data = get_real_time_ocean_data(*self.current_location)
        
    @st.cache_resource
    def load_model(_self):
        """Load custom YOLO model with cache"""
        try:
            model = YOLO('models/tn_coastal_v8.pt')
            model.conf = 0.7
            return model
        except:
            try:
